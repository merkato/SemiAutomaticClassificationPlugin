...
class ClassificationDock:
	...
	def algorithmName(self):
		...
		cfg.utls.writeProjectVariable("ClassAlgorithm", cfg.algName)
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "training name: " + cfg.algName)
	...
	def algorithmThreshold(self):
		...
		cfg.algThrshld = cfg.uidc.alg_threshold_SpinBox.value()
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "algorithm threshold: " + cfg.algThrshld)
	...
	def applyQmlStyle(self, classLayer, stylePath):
		...
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "classification symbology applied with qml: " + stylePath)
	...
	def createPreview(self, point, algorithmRaster = "No"):
		...
		if cfg.pntPrvw != None:
			...
			# logger
			cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), ">>> PREVIEW click")
			...
			pP = cfg.tmpDir + "/" + pN
			# logger
			cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "point (X,Y) = (%s,%s)" % (cfg.pntPrvw.x() , cfg.pntPrvw.y()))
			# signature list
				...
				if ckB == "Yes":
					...
					if ok == "Yes":
						if algorithmRaster == "No":
							r = cfg.iface.addRasterLayer(pP, cfg.osSCP.path.basename(pP))
							cfg.lastPrev = r.name()
							cfg.uiUtls.updateBar(80)
							# apply symbology
							self.applyClassSymbology(r, cfg.macroclassCheck, cfg.qmlFl, sL)
						else:
							r = cfg.iface.addRasterLayer(mOut, cfg.osSCP.path.basename(mOut))
							cfg.lastPrev = r.name()
					...
					# logger
					cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "<<< PREVIEW created: " + unicode(pN))
					# enable Redo button
					...
					cfg.mx.msgErr6()
					# logger
					cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "preview no")
			else:
				cfg.uiUtls.removeProgressBar()
	...
	def getSignatureList(self):
		...
		for i in cfg.ROI_SCP_UID.values():
			ifi not in cfg.signIDs.values() and cfg.signList["CHECKBOX_" +i] == 2:
				rId = cfg.utls.getIDByAttributes(cfg.shpLay, cfg.fldSCP_UID,i)
		...
		signatureList = []
		for i in id:
			if cfg.signList["CHECKBOX_" +i] == 2:
				s = []
				s.append(cfg.signList["MACROCLASSID_" +i])
				s.append(cfg.signList["MACROCLASSINFO_" +i])
				s.append(cfg.signList["CLASSID_" +i])
				s.append(cfg.signList["CLASSINFO_" +i])
				s.append(cfg.signList["VALUES_" +i])
				s.append(cfg.signList["WAVELENGTH_" +i])
				s.append(cfg.signList["COLOR_" +i])
				s.append(cfg.signList["COVMATRIX_" +i])
				s.append(cfg.signList["LCS_MIN_" +i])
				s.append(cfg.signList["LCS_MAX_" +i])
				if len(cfg.signList["WAVELENGTH_" +i]) == len(cfg.bndSetWvLn.values()):
					if sorted(cfg.signList["WAVELENGTH_" +i]) != sorted(cfg.bndSetWvLn.values()):
						cfg.mx.msgWar9(cfg.signList["MACROCLASSID_" +i], cfg.signList["CLASSID_" +i])
					# check if signature has covariance matrix if maximum likelihood
					if cfg.algName == cfg.algML:
						if cfg.signList["COVMATRIX_" +i] == "No":
							cfg.mx.msgWar10(cfg.signList["MACROCLASSID_" +i], cfg.signList["CLASSID_" +i])
						else:
							signatureList.append(s)
					else:
						signatureList.append(s)
				else:
					cfg.mx.msgErr24(cfg.signList["MACROCLASSID_" +i], cfg.signList["CLASSID_" +i])
					self.trainSigCheck = "No"
					return None
				if cfg.algName == cfg.algMinDist:
					s.append(cfg.signList["MD_THRESHOLD_" +i])
				elif cfg.algName == cfg.algML:
					s.append(cfg.signList["ML_THRESHOLD_" +i])
				elif cfg.algName == cfg.algSAM:
					s.append(cfg.signList["SAM_THRESHOLD_" +i])
	...
	def macroclassCheckbox(self):
		...
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " checkbox set: " + cfg.macroclassCheck)
	...
	def LCSignature_Checkbox(self):
		...
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " checkbox set: " + str(cfg.LCsignatureCheckBox))
	...
	def maskCheckbox(self):
		if cfg.uidc.mask_checkBox.isChecked() is True:
			...
			if len(m) > 0:
				cfg.mskFlPath = m
				cfg.uidc.mask_lineEdit.setText(cfg.mskFlPath)
				cfg.mskFlState = 2
			else:
				cfg.mskFlState = 2
			...
			cfg.mskFlState = 0
		cfg.utls.writeProjectVariable("maskFilePath", cfg.mskFlPath)
		cfg.utls.writeProjectVariable("maskFileState", cfg.mskFlState)
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " checkbox set: " + str(cfg.mskFlState))
	...
	def resetMask(self):
		...
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "reset mask")
	...
	def setMaskCheckbox(self):
		...
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "mask checkbox")
	...
	def pointerPreviewActive(self):
		...
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "pointer active: preview")
	...
	def previewSize(self):
		...
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "preview size: " + str(cfg.prvwSz))
	...
	def redoPreview(self):
		...
		if cfg.actionCheck == "No":
			if cfg.pntPrvw is None:
				...
				self.createPreview(cfg.pntPrvw, self.algRasterPrevw)
				# logger
				cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "REDO Preview")
	...
	def reportCheckbox(self):
		if cfg.uidc.report_checkBox.isChecked() is True:
			...
			cfg.reportCheck = "No"
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " checkbox set: " + str(cfg.reportCheck))
	...
	def resetQmlStyle(self):
		...
		cfg.qmlFl = ""
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "reset qml")
	...
	def runAlgorithm(self, algorithmName, imageName, signatureList, outputRasterPath, macroclassCheck = "No", algRasterPath = None, previewSize = 0, previewPoint = None, compress = "No"):
		...
		if cfg.bndSetPresent == "Yes" and cfg.imgNm == cfg.bndSetNm:
			...
			for i in range(0, len(bS)):
				rD = cfg.gdalSCP.Open(bS[i], cfg.gdalSCP.GA_ReadOnly)
				bL.append(rD)
		else:
			# if masked raster
			if imageName == cfg.maskRasterNm:
		...
		if rD is not None:
			...
			if cfg.rasterCompression != "No":
				try:
					...
					cfg.osSCP.remove(tPMD2)
					# logger
					if cfg.logSetVal == "Yes": cfg.utls.logToFile(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " ERROR exception: " + err)
			else:
				cfg.shutilSCP.copy(tPMD2, outputRasterPath)
				cfg.osSCP.remove(tPMD2)
			...
			cfg.mx.msgErr25()
			# logger
			cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " error raster")
			return "No", None, None
	...
	def selectAllSignatures(self):
		try:
			...
			for row in range(0, c):
				...
				cfg.signList["CHECKBOX_" + id] = tW.item(row, 0).checkState()
			cfg.uiUtls.removeProgressBar()
			# logger
			cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " all signatures")
		except Exception as err:
			# logger
			cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " ERROR exception: " + err)
			cfg.uiUtls.removeProgressBar()
	...
	def runClassification(self, batch = "No", outputClassification = None):
		...
		if self.trainSigCheck == "Yes":
			...
			if len(clssOut) > 0:
				...
				clssOut = clssOut.replace('//', '/')
				cfg.clssPth = clssOut
				sN = cfg.osSCP.path.basename(clssOut)
				if sN.endswith(".tif"):
					cfg.clssPth = clssOut
				else:
			...
			# logger
			cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "classification output: " + cfg.clssPth)
			...
			if ckC != "No":
				# logger
				cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), ">>> CLASSIFICATION STARTED")
				# base name
					...
					if cfg.uidc.mask_checkBox.isChecked() is True:
						...
						if cfg.bndSetPresent == "Yes" and cfg.imgNm == cfg.bndSetNm:
							for x in range(0, len(cfg.bndSetLst)):
								tCD = cfg.tmpDir + "/" + x + tCN
								cfg.bndSetMaskList.append(tCD)
								cfg.utls.clipRasterByShapefile(m, cfg.bndSetLst[x], tCD, cfg.outTempRastFormat)
						else:
							# temp masked raster
							cfg.maskRstSrc = cfg.tmpDir + "/" + tCN
							cfg.utls.clipRasterByShapefile(m, cfg.imgSrc, cfg.maskRstSrc, cfg.outTempRastFormat)
				...
				### if not mask
					...
					if ok == "Yes":
						c = cfg.iface.addRasterLayer(cfg.clssPth, cfg.osSCP.path.basename(cfg.clssPth))
						...
						# logger
						cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "<<< CLASSIFICATION PERFORMED: " + (cfg.clssPth)
				### calculate report
				...
				### convert classification to vector
					...
					if cfg.vectorOutputCheck == "Yes":
						...
						vl = cfg.utls.addVectorLayer(vO, cfg.osSCP.path.basename(vO), "ogr")
				...
				### copy signature raster
					if cfg.algFilesCheck == "Yes":
						...
						try:
							...
							if cfg.uidc.LC_signature_checkBox.isChecked():
								cfg.utls.rasterSymbolLCSAlgorithmRaster(c)
							# logger
							cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "files copied")
						except Exception, err:
							# logger
							cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " ERROR exception: " + err)
							cfg.mx.msgErr23()
					try:
						...
						# logger
						cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "files deleted")
					except Exception as err:
						# logger
						cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " ERROR exception: " + err)
				### ending
			...
			### band set check failed
					...
					# logger
					cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "band set check failed")
		else:
			cfg.mx.msg18()
			if batch == "No":
				cfg.cnvs.setRenderFlag(True)
			# logger
			cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "classification no")
	...
	def selectQmlStyle(self):
		...
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "qml file: " + cfg.qmlFl)
	...
	def openSignatureList(self, path = None):
		...
		if len(signFilePath) > 0:
			...
			# logger
			cfg.utls.logCondition(__name__ + "-" + (cfg.inspectSCP.stack()[0][3])+ " " + cfg.utls.lineOfCode(), " signatures opened: " + signFilePath)
	...
	def openInput(self, scpPath = None):
		...
		if check == "Yes":
			cfg.utls.writeProjectVariable("trainingLayer", scpPath)
			cfg.scpFlPath = scpPath
			cfg.uidc.trainingFile_lineEdit.setText(scpPath)
			# logger
			cfg.utls.logCondition(__name__ + "-" + (cfg.inspectSCP.stack()[0][3])+ " " + cfg.utls.lineOfCode(), " signatures opened: " + scpPath)
		else:
			# shape layer
			cfg.shpLay = None
			# training layer name
	...
	def openShapeFile(self, shapeFilePath):
		# shapefile
		zipName = cfg.osSCP.path.basename(shapeFilePath)
		name = zipName[:-4]
		dT = cfg.utls.getTime()
		...
		try:
			...
			cfg.mx.msgErr59()
			# logger
			cfg.utls.logCondition(__name__ + "-" + (cfg.inspectSCP.stack()[0][3])+ " " + cfg.utls.lineOfCode(), "Error training input")
			return "No"
		...
		if check == "Yes":
			...
			mL = QgsVectorLayer("MultiPolygon?crs=" + pCrs.toWkt(), name, "memory")
	...
	def addFieldsToLayer(self, layer):
		...
		layer.commitChanges()
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " fields added")
		...
		try:
			...
			for child in root:
				try:
					...
					b = cfg.utls.signatureID()
				cfg.signList["MACROCLASSID_" +b] = child.find("MACROCLASSID").text.strip()
				cfg.signList["MACROCLASSINFO_" +b] = child.find("MACROCLASSINFO").text.strip()
				cfg.signList["CLASSID_" +b] = child.find("CLASSID").text.strip()
				cfg.signList["CLASSINFO_" +b] = child.find("CLASSINFO").text.strip()
				cfg.signList["UNIT_" +b] = child.find("WAVELENGTH_UNIT").text.strip()
				cfg.signList["ROI_SIZE_" +b] = child.find("ROI_SIZE").text.strip()
				cfg.signIDs["ID_" +b] = b
				# get values
				vls = child.find("VALUES").text.strip()
				x = eval(vls)
				cfg.signList["VALUES_" +b] = x
				try:
					lcsMin = child.find("LCS_MIN").text.strip()
					min = eval(lcsMin)
					cfg.signList["LCS_MIN_" +b] = min
					lcsMax = child.find("LCS_MAX").text.strip()
					max = eval(lcsMax)
					cfg.signList["LCS_MAX_" +b] = max
				except:
					cfg.signList["LCS_MIN_" +b] = x
					cfg.signList["LCS_MAX_" +b] = x
				try:
					minV = child.find("MIN_VALUE").text.strip()
					minVal = eval(minV)
					cfg.signList["MIN_VALUE_" +b] = minVal
					maxV = child.find("MAX_VALUE").text.strip()
					maxVal = eval(maxV)
					cfg.signList["MAX_VALUE_" +b] = maxVal
				except:
					cfg.signList["MIN_VALUE_" +b] = x
					cfg.signList["MAX_VALUE_" +b] = x
				cfg.signList["WAVELENGTH_" +b] = eval(child.find("WAVELENGTH").text.strip())
				cfg.signList["SD_" +b] = eval(child.find("SD").text.strip())
				cfg.signList["MEAN_VALUE_" +b] = eval(child.find("MEAN_VALUE").text.strip())
				try:
					cfg.signList["CHECKBOX_" +b] = eval(child.find("CHECKBOX").text.strip())
				except:
					cfg.signList["CHECKBOX_" +b] = 2
				c = cfg.QtGuiSCP.QColor()
				c.setNamedColor(child.find("COLOR").text.strip())
				cfg.signList["COLOR_" +b] = c
				# get covariance matrix
				mt = child.find("COVARIANCE_MATRIX").text.strip()
				try:
					cm = eval(mt)
				except:
					cm = "No"
				cfg.signList["COVMATRIX_" +b] = cfg.utls.listToCovarianceMatrix(cm)
				try:
					cfg.signList["MD_THRESHOLD_" +b] = float((child.find("SIGNATURE_THRESHOLD_MD").text).strip())
				except:
					cfg.signList["MD_THRESHOLD_" +b] = 0
				try:
					cfg.signList["ML_THRESHOLD_" +b] = float((child.find("SIGNATURE_THRESHOLD_ML").text).strip())
				except:
					cfg.signList["ML_THRESHOLD_" +b] = 0
				try:
					cfg.signList["SAM_THRESHOLD_" +b] = float((child.find("SIGNATURE_THRESHOLD_SAM").text).strip())
				except:
					cfg.signList["SAM_THRESHOLD_" + b] = 0
			cfg.classD.ROIListTable(cfg.trnLay, cfg.uidc.signature_list_tableWidget)
			# logger
			if cfg.logSetVal == "Yes": cfg.utls.logToFile(__name__ + "-" + (cfg.inspectSCP.stack()[0][3])+ " " + cfg.utls.lineOfCode(), " opened signature " + len(cfg.signIDs))
		except Exception, err:
			cfg.classD.ROIListTable(cfg.trnLay, cfg.uidc.signature_list_tableWidget)
			cfg.mx.msgErr16()
			# logger
	...
	def exportSignatureFile(self):
		...
		if len(signFile) > 0:
			...
			if signFile.lower().endswith(".scp"):
				...
				sL = signFile + ".scp"
			# shapefile
			zipName = cfg.osSCP.path.basename(sL)
			name = zipName[:-4]
			dT = cfg.utls.getTime()
			unzipDir = cfg.tmpDir + "/" + name + dT
			shpF = unzipDir + "/" + name + ".shp"
			sigFile = unzipDir + "/" + name + ".slf"
			oDir = cfg.utls.makeDirectory(unzipDir)
			QgsVectorFileWriter(shpF, "CP1250", f, QGis.WKBMultiPolygon , crs, "ESRI Shapefile")
			...
			# logger
			cfg.utls.logCondition(__name__ + "-" + (cfg.inspectSCP.stack()[0][3])+ " " + cfg.utls.lineOfCode(), " signatures exported in: " + signFile)
	...
	def saveSignatureListToFile(self):
		try:
			cfg.classD.saveSignatureList(cfg.sigFile)
			# logger
			cfg.utls.logCondition(__name__ + "-" + (cfg.inspectSCP.stack()[0][3])+ " " + cfg.utls.lineOfCode(), " signatures saved in: " + cfg.sigFile)
		except Exception as err:
			cfg.mx.msgErr15()
			# logger
			cfg.utls.logCondition(__name__ + "-" + (cfg.inspectSCP.stack()[0][3])+ " " + cfg.utls.lineOfCode(), " ERROR exception: " + err)
		...
		try:
			if cfg.bndSetUnit["UNIT"] != cfg.noUnit:
				...
				if len(libFile) > 0:
					...
					cfg.uiUtls.removeProgressBar()
					# logger
					cfg.utls.logCondition(__name__ + "-" + (cfg.inspectSCP.stack()[0][3])+ " " + cfg.utls.lineOfCode(), " spectral library " + libFile)
			else:
				cfg.mx.msgWar8()
		except Exception as err:
			cfg.mx.msgWar8()
		...
		if len(r) > 0:
			...
			if len(d) > 0:
				for b in v:
					...
					c = tW.item(b, 4).text()
					signFile = d + "/" + mC + "_" + mID + "_" + c + "_" + cID + ".csv"
					# open file
					l = open(signFile, 'w')
					try:
						...
						cfg.utls.logCondition(__name__ + "-" + (cfg.inspectSCP.stack()[0][3])+ " " + cfg.utls.lineOfCode(), " ERROR exception: " + err)
					id = tW.item(b, 6).text()
					u = cfg.signList["UNIT_" + id]
					# wavelength
					a = cfg.signList["WAVELENGTH_" + id]
					wlg = eval(a)
					# signature values
					n = cfg.signList["VALUES_" + id]
					val = eval(n)
					# open file
					l = open(signFile, 'a')
	...
	def vectorCheckbox(self):
		...
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " checkbox set: " + cfg.vectorOutputCheck)
...
##################################
		...
		if a == "Yes":
			...
			for x in v:
				...
				if id in cfg.ROI_SCP_UID.values():
					rId = cfg.utls.getIDByAttributes(cfg.shpLay, cfg.fldSCP_UID, id)
		...
		if len(set(r)) > 1:
			...
			if a == "Yes":
				...
				for x in v:
					...
					# if ROI
					if id in cfg.ROI_SCP_UID.values():
						ifd = cfg.utls.getIDByAttributes(cfg.shpLay, cfg.fldSCP_UID, id)
						ids.append(ifd[0])
						ROIcheck.append(1)
					else:
						ROIcheck.append(0)
					# if not signatue
					if id not in cfg.signIDs.values():
						rId = cfg.utls.getIDByAttributes(cfg.shpLay, cfg.fldSCP_UID, id)
					...
					if len(wl) == 0:
						wl = cfg.signList["WAVELENGTH_" + id]
						unit = cfg.signList["UNIT_" + id]
						MC_ID  = cfg.signList["MACROCLASSID_" + id]
						MC_Info  = cfg.merged_name + cfg.signList["MACROCLASSINFO_" + id]
						C_ID = cfg.signList["CLASSID_" + id]
						C_Info = cfg.merged_name + cfg.signList["CLASSINFO_" + id]
						color = cfg.signList["COLOR_" + id]
						checkbox  = cfg.signList["CHECKBOX_" + id]
						sigThr  = cfg.signList["MD_THRESHOLD_" + id]
						sigThrML  = cfg.signList["ML_THRESHOLD_" + id]
						sigThrSAM  = cfg.signList["SAM_THRESHOLD_" + id]
					elif wl != cfg.signList["WAVELENGTH_" + id] or unit != cfg.signList["UNIT_" + id]:
						cfg.mx.msgErr35()
						return "No"
					val.append(cfg.signList["VALUES_" + id])
					min.append(cfg.signList["LCS_MIN_" + id])
					max.append(cfg.signList["LCS_MAX_" + id])
					vmin.append(cfg.signList["MIN_VALUE_" + id])
					vmax.append(cfg.signList["MAX_VALUE_" + id])
					covMat.append(cfg.signList["COVMATRIX_" + id])
				...
				if 0 not in ROIcheck:
					...
					cfg.signList["CHECKBOX_" +i] = cfg.QtSCP.Checked
					cfg.signList["MACROCLASSID_" +i] = MC_ID
					cfg.signList["MACROCLASSINFO_" +i] = MC_Info
					cfg.signList["CLASSID_" +i] = C_ID
					cfg.signList["CLASSINFO_" +i] = C_Info
					cfg.signList["WAVELENGTH_" +i] = wl
					cfg.signList["VALUES_" +i] = val_mean
					cfg.signList["LCS_MIN_" +i] = min_mean
					cfg.signList["LCS_MAX_" +i] = max_mean
					cfg.signList["MIN_VALUE_" +i] = vmin_mean
					cfg.signList["MAX_VALUE_" +i] = vmaxs_mean
					cfg.signList["ROI_SIZE_" +i] = 0
					cfg.signList["COVMATRIX_" +i] = covMatrix
					cfg.signList["MD_THRESHOLD_" +i] = sigThr
					cfg.signList["ML_THRESHOLD_" +i] = sigThrML
					cfg.signList["SAM_THRESHOLD_" +i] = sigThrSAM
					# counter
					n = 0
					m = []
					sdL = []
					for wi in wl:
						...
						n = n + 1
					cfg.signList["MEAN_VALUE_" +i] = m
					cfg.signList["SD_" +i] = sdL
					if unit is None:
						unit = cfg.bndSetUnit["UNIT"]
					cfg.signList["UNIT_" +i] = unit
					cfg.signList["COLOR_" +i] = color
					cfg.signIDs["ID_" +i] = i
				cfg.classD.ROIListTable(cfg.trnLay, cfg.uidc.signature_list_tableWidget)
				# select row
				...
				cfg.uiUtls.removeProgressBar()
				# logger
				cfg.utls.logCondition(__name__ + "-" + (cfg.inspectSCP.stack()[0][3])+ " " + cfg.utls.lineOfCode(), " merged signatures: " + v)
	...
	def removeSelectedSignatures(self):
		...
		if len(tW.selectedIndexes()) > 0:
			...
			if a == "Yes":
				...
				cfg.classD.ROIListTable(cfg.trnLay, cfg.uidc.signature_list_tableWidget)
				# logger
				cfg.utls.logCondition(__name__ + "-" + (cfg.inspectSCP.stack()[0][3])+ " " + cfg.utls.lineOfCode(), " removed signatures: " + v)

	# delete signature by ID
	def deleteSignatureByID(self, id):
		cfg.signIDs.pop("ID_" + id)
		cfg.signList.pop("MACROCLASSID_" + id)
		cfg.signList.pop("MACROCLASSINFO_" + id)
		cfg.signList.pop("CLASSID_" + id)
		cfg.signList.pop("CLASSINFO_" + id)
		cfg.signList.pop("WAVELENGTH_" + id)
		try:
			cfg.signList.pop("MEAN_VALUE_" + id)
			cfg.signList.pop("SD_" + id)
		except:
			pass
		cfg.signList.pop("VALUES_" + id)
		cfg.signList.pop("LCS_MIN_" + id)
		cfg.signList.pop("LCS_MAX_" + id)
		cfg.signList.pop("MIN_VALUE_" + id)
		cfg.signList.pop("MAX_VALUE_" + id)
		cfg.signList.pop("ROI_SIZE_" + id)
		cfg.signList.pop("COLOR_" + id)
		cfg.signList.pop("UNIT_" + id)
		cfg.signList.pop("COVMATRIX_" + id)
		cfg.signList.pop("MD_THRESHOLD_" + id)
		cfg.signList.pop("ML_THRESHOLD_" + id)
		cfg.signList.pop("SAM_THRESHOLD_" + id)
		try:
	...
	def addROIToScatterPlot(self):
		...
		if len(v) > 0:
			...
			for x in v:
				...
				id = tW.item(x, 6).text()
				if id in cfg.ROI_SCP_UID.values():
					h = cfg.utls.calculateScatterPlot(cfg.shpLay, cfg.fldSCP_UID, id)
	...
	def sigListToPlot(self, id):
		cfg.signPlotIDs["ID_" + id] = id
		cfg.spectrPlotList["MACROCLASSID_" + id] = cfg.signList["MACROCLASSID_" + id]
		cfg.spectrPlotList["MACROCLASSINFO_" + id] = cfg.signList["MACROCLASSINFO_" + id]
		cfg.spectrPlotList["CLASSID_" + id] = cfg.signList["CLASSID_" + id]
		cfg.spectrPlotList["CLASSINFO_" + id] = cfg.signList["CLASSINFO_" + id]
		cfg.spectrPlotList["VALUES_" + id] = cfg.signList["VALUES_" + id]
		cfg.spectrPlotList["ROI_SIZE_" + id] = cfg.signList["ROI_SIZE_" + id]
		cfg.spectrPlotList["MIN_VALUE_" + id] = cfg.signList["MIN_VALUE_" + id]
		cfg.spectrPlotList["MAX_VALUE_" + id] = cfg.signList["MAX_VALUE_" + id]
		cfg.spectrPlotList["LCS_MIN_" + id] = cfg.signList["LCS_MIN_" + id]
		cfg.spectrPlotList["LCS_MAX_" + id] = cfg.signList["LCS_MAX_" + id]
		cfg.spectrPlotList["WAVELENGTH_" + id] = cfg.signList["WAVELENGTH_" + id]
		cfg.spectrPlotList["MEAN_VALUE_" + id] = cfg.signList["MEAN_VALUE_" + id]
		cfg.spectrPlotList["SD_" + id] = cfg.signList["SD_" + id]
		cfg.spectrPlotList["COLOR_" + id] = cfg.signList["COLOR_" + id]
		cfg.spectrPlotList["CHECKBOX_" + id] = 2
		cfg.spectrPlotList["UNIT_" + id] = cfg.signList["UNIT_" + id]
		cfg.spectrPlotList["COVMATRIX_" + id] = cfg.signList["COVMATRIX_" + id]
		cfg.spectrPlotList["MD_THRESHOLD_" + id] = cfg.signList["MD_THRESHOLD_" + id]
		cfg.spectrPlotList["ML_THRESHOLD_" + id] = cfg.signList["ML_THRESHOLD_" + id]
		cfg.spectrPlotList["SAM_THRESHOLD_" + id] = cfg.signList["SAM_THRESHOLD_" + id]
		...
		if l is not None:
			...
			for f in l.getFeatures():
				...
				try:
					...
					cfg.ROI_MC_ID[SCP_UID]= f[cfg.fldMacroID_class]
					cfg.ROI_MC_Info[SCP_UID] = f[cfg.fldROIMC_info]
					cfg.ROI_C_ID[SCP_UID] = f[cfg.fldID_class]
					cfg.ROI_C_Info[SCP_UID] = f[cfg.fldROI_info]
					cfg.ROI_Count[SCP_UID]= i
					cfg.ROI_ShapeID[SCP_UID]= id
				except Exception, err:
					# logger
					cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " ERROR exception: " + err)
					cfg.mx.msg3()
				i = i + 1
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " ROI attributes")
	...
	def ROIListTable(self, layerName, table, checkstate=0):
		...
		try:
			for k in sorted(cfg.ROI_SCP_UID.values()):
				...
				if k in cfg.signIDs.values():
					cfg.utls.addTableItem(l, "checkbox", b, 0, "Yes", None, cfg.signList["CHECKBOX_" + k])
					cfg.utls.addTableItem(l, cfg.ROISigTypeNm, b, 1, "No")
					cfg.utls.addTableItem(l, "", b, 5, "Yes", cfg.signList["COLOR_" + k])
					# for signature list coherence
					try:
						cfg.signList["MACROCLASSID_" + k] = int(cfg.ROI_MC_ID[k])
					except:
						cfg.signList["MACROCLASSID_" + k] = int(0)
					cfg.signList["MACROCLASSINFO_" + k] = cfg.ROI_MC_Info[k]
					try:
						cfg.signList["CLASSID_" + k] = int(cfg.ROI_C_ID[k])
					except:
						cfg.signList["CLASSID_" + k] = int(0)
					cfg.signList["CLASSINFO_" + k] = cfg.ROI_C_Info[k]
				else:
					try:
						cfg.utls.addTableItem(l, "checkbox", b, 0, "Yes", None, cfg.signList["CHECKBOX_" + k])
					except:
						cfg.utls.addTableItem(l, "checkbox", b, 0, "Yes", None, cfg.QtSCP.Checked)
						cfg.signList["CHECKBOX_" + k] = 2
				...
				cfg.utls.addTableItem(l, cfg.ROI_C_Info[k], b, 4, "Yes", None, None, cfg.ROI_C_Info[k])
				cfg.utls.addTableItem(l, k, b, 6)
				b = b +1
		except Exception as err:
			# logger
			cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " ERROR exception: " + err)
		...
		for k in sorted(cfg.signIDs.values()):
			if k not in cfg.ROI_SCP_UID.values():
				cfg.utls.insertTableRow(l, b, 20)
				cfg.utls.addTableItem(l, "checkbox", b, 0, "Yes", None, cfg.signList["CHECKBOX_" + k])
				cfg.utls.addTableItem(l, cfg.SIGTypeNm, b, 1, "No")
				cfg.utls.addTableItem(l, "", b, 5, "Yes", cfg.signList["COLOR_" + k])
				cfg.utls.addTableItem(l, int(cfg.signList["MACROCLASSID_" + k]), b, 2)
				cfg.utls.addTableItem(l, int(cfg.signList["CLASSID_" + k]), b, 3)
				cfg.utls.addTableItem(l, cfg.signList["CLASSINFO_" + k], b, 4, "Yes", None, None, cfg.signList["CLASSINFO_" + k])
				cfg.utls.addTableItem(l, k, b, 6)
				b = b +1
		l.show()
		...
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " roi list table created")
...
##################################
	...
	def rapidROIband(self):
		...
		cfg.utls.writeProjectVariable("rapidROIBand", cfg.ROIband)
		# auto refresh ROI
		if cfg.uidc.auto_refresh_ROI_radioButton.isChecked() and cfg.ROITime is not None:
			...
			if StartT > (cfg.ROITime + cfg.datetimeSCP.timedelta(seconds=1)):
				self.redoROI()
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "ROI band: " + cfg.ROIband)
	...
	def rapidROICheckbox(self):
		...
		if cfg.uidc.auto_refresh_ROI_radioButton.isChecked() and cfg.ROITime is not None:
			...
			if StartT > (cfg.ROITime + cfg.datetimeSCP.timedelta(seconds=1)):
				self.redoROI()
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " checkbox set: " + cfg.rpdROICheck)

	# set vegetation index name
	def vegetationIndexName(self):
		cfg.indName = cfg.uidc.vegetation_index_comboBox.currentText()
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "index name: " + cfg.indName)
	...
	def vegetationIndexCheckbox(self):
		...
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " checkbox set: " + cfg.vegIndexCheck)

	# set ROI macroclass ID
	def setROIMacroID(self):
		cfg.ROIMacroID = cfg.uidc.ROI_Macroclass_ID_spin.value()
		cfg.utls.writeProjectVariable("ROIMacroIDField", cfg.ROIMacroID)
		for i in cfg.MCID_List:
			if cfg.ROIMacroID == i[0]:
				cfg.uidc.ROI_Macroclass_line.setText(i[1])
				cfg.ROIMacroClassInfo = i[1]
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "roi macroclass id: " + cfg.ROIMacroID)

	# set ROI class ID
	def setROIID(self):
		cfg.ROIID = cfg.uidc.ROI_ID_spin.value()
		cfg.utls.writeProjectVariable("ROIIDField", cfg.ROIID)
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "roi id: " + cfg.ROIID)

	# set ROI class info
	def roiClassInfo(self):
		cfg.uidc.ROI_Class_line.blockSignals(True)
		iTxt = cfg.uidc.ROI_Class_line.text().encode('ascii','replace')
		cfg.ROIInfo = iTxt
		cfg.utls.writeProjectVariable("ROIInfoField", cfg.ROIInfo)
		cfg.uidc.ROI_Class_line.blockSignals(False)
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "roi info: " + cfg.ROIInfo)
	...
	def customExpressionEdited(self):
		cfg.customExpression = cfg.uidc.custom_index_lineEdit.text()
		cfg.utls.writeProjectVariable("customExpression", cfg.customExpression)
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " expression: " + cfg.customExpression)

	# set ROI class info
	def roiMacroclassInfo(self):
		cfg.uidc.ROI_Macroclass_line.blockSignals(True)
		iTxt = cfg.uidc.ROI_Macroclass_line.text().encode('ascii','replace')
		cfg.ROIMacroClassInfo = iTxt
		cfg.utls.writeProjectVariable("ROIMacroclassInfoField", cfg.ROIMacroClassInfo)
	...
	def editedCell(self, row, column):
		...
		id = tW.item(row, 6).text()
		if column == 0:
			cfg.signList["CHECKBOX_" + id] = tW.item(row, column).checkState()
		elif column == 5:
			...
			try:
				...
				if v < 0:
					v = 0
					cfg.utls.setTableItem(tW, row, column, v)
					cfg.mx.msg17()
				cfg.signList["MACROCLASSID_" + id] = v
				if id in cfg.ROI_SCP_UID.values():
					cfg.ROI_MC_ID[id] = v
					rId = cfg.utls.getIDByAttributes(cfg.shpLay, cfg.fldSCP_UID, id)
					for rI in rId:
				...
				tW.setItem(row, column, QTableWidgetItem(cfg.signList["MACROCLASSID_" + id]))
		elif column == 3:
			try:
				v = int(tW.item(row, column).text())
				if v < 0:
				...
				tW.setItem(row, column, QTableWidgetItem(cfg.signList["CLASSID_" + id]))
		elif column == 4:
			iTxt2 = tW.item(row, column).text().encode('ascii','replace')
			cfg.signList["CLASSINFO_" + id] = iTxt2
		...
		cfg.LCSignT.LCSignatureThresholdListTable()
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "edited cell" + row + ";" + column)
		...
		if index.column() == 2:
			...
			if c is not None:
				...
				for x in v:
					...
					tW.clearSelection()
		# logger
		cfg.utls.logCondition(__name__ + "-" + (cfg.inspectSCP.stack()[0][3])+ " " + cfg.utls.lineOfCode(), " index: " + index)
	...
	def addMCIDToTable(self):
		...
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "added row " + c)
	...
	def signatureListDoubleClick(self, index):
		...
		# logger
		cfg.utls.logCondition(__name__ + "-" + (cfg.inspectSCP.stack()[0][3])+ " " + cfg.utls.lineOfCode(), " signatures index: " + index)
	...
	def zoomToROI(self, index):
		...
		if l is not None:
			...
			if id in cfg.ROI_SCP_UID.values():
				rId = cfg.utls.getIDByAttributes(l, cfg.fldSCP_UID, id)
	...
	def signatureCheckbox(self):
		...
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " checkbox set: " + cfg.sigClcCheck)
...
##################################
	...
	def createInput(self):
		# logger
		cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), ">>> create input click")
		try:
			if cfg.rstrNm is not None:
				...
				if len(sF) > 0:
					try:
						...
						# logger
						cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "<<< SCP created: " + "\"" + sL + "\"")
					except Exception, err:
						# logger
						cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " ERROR exception: " + err)
						cfg.ipt.refreshRasterLayer()
				...
				cfg.mx.msg4()
				# logger
				cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "<<< create SCP fail: no raster")
				cfg.ipt.refreshRasterLayer()
		except Exception, err:
			# logger
			cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), " ERROR exception: " + err)
	...
	def saveROItoShapefile(self, progressbar = "Yes"):
		...
		# check if no ROI created
			...
			try:
				...
				fdID = cfg.utls.fieldID(cfg.shpLay, cfg.fldID_class)
				cfg.shpLay.changeAttributeValue(self.ROILastID, fdID, cfg.ROIID)
				# set macroclass ID attribute
				fdMID = cfg.utls.fieldID(cfg.shpLay, cfg.fldMacroID_class)
			...
			fdInfo = cfg.utls.fieldID(cfg.shpLay, cfg.fldROI_info)
			cfg.shpLay.changeAttributeValue(self.ROILastID, fdInfo, cfg.ROIInfo)
			# set ROI Macroclass info attribute
			fdMCInfo = cfg.utls.fieldID(cfg.shpLay, cfg.fldROIMC_info)
			...
			SCP_UID = cfg.utls.fieldID(cfg.shpLay, cfg.fldSCP_UID)
			...
			# logger
			cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "roi: " + cfg.ROIID + ", " + cfg.ROIInfo + " saved to shapefile: " + cfg.shpLay.name())
	...
	def undoSaveROI(self):
		...
		# check if no ROI created
			...
			if a == "Yes":
				...
				cfg.cnvs.refresh()
				# logger
				cfg.utls.logCondition(__name__ + "-" + cfg.inspectSCP.stack()[0][3]+ " " + cfg.utls.lineOfCode(), "roi deleted: " + self.ROILastID)
